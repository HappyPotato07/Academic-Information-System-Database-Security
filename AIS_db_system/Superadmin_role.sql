...
CREATE USER Admin1 FOR LOGIN A001;        -- Database User for Admin1
CREATE USER Admin2 FOR LOGIN A002;        -- Database User for Admin2
CREATE USER Lecturer1 FOR LOGIN L001;     -- Database User for Lecturer1
CREATE USER Lecturer2 FOR LOGIN L002;     -- Database User for Lecturer2
CREATE USER Student1 FOR LOGIN S001;      -- Database User for Student1
CREATE USER Student2 FOR LOGIN S002;      -- Database User for Student2
GO

-- Add Users to their respective Roles --
ALTER ROLE DataAdmin ADD MEMBER Admin1;   -- Add Admin1 to DataAdmin Role
ALTER ROLE DataAdmin ADD MEMBER Admin2;   -- Add Admin2 to DataAdmin Role
ALTER ROLE Lecturer ADD MEMBER Lecturer1; -- Add Lecturer1 to Lecturer Role
ALTER ROLE Lecturer ADD MEMBER Lecturer2; -- Add Lecturer2 to Lecturer Role
ALTER ROLE Student ADD MEMBER Student1;   -- Add Student1 to Student Role
ALTER ROLE Student ADD MEMBER Student2;   -- Add Student2 to Student Role
GO
...
-- student table
CREATE TRIGGER trg_AuditStudentModifications
ON Student
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
...
-- lecturer table
CREATE TRIGGER trg_AuditLecturerModifications
ON Lecturer
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
...
-- subject table
CREATE TRIGGER trg_AuditSubjectModifications
ON Subject
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
...
CREATE TRIGGER trg_PreventDropObject
ON DATABASE
...
VALUES (
        ENCRYPTBYKEY(KEY_GUID('LecturerIDKey'), 'L005'),  -- Encrypt Lecturer ID
        ENCRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), 'Lecturer123'),  -- Encrypt password
        'Lecturer5',
        '013-4555555',  -- Phone (masked automatically)
        'IT'),
       (
        ENCRYPTBYKEY(KEY_GUID('LecturerIDKey'), 'L006'),
        ENCRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), 'Lecturer456'),  -- Encrypt password
        'Lecturer6',
        '012-3412312',  -- Phone (masked automatically)
        'Economics');
GO
...
-- Grant full access to Result table
...
GRANT ALTER ANY LOGIN TO S004;  -- Grant to S002 (Student2)
GO
...
GRANT ALTER ANY USER TO Admin1;  -- Allow Admin1 (A001) to alter users
GRANT ALTER ANY USER TO Admin2;  -- Allow Admin2 (A002) to alter users
...
GRANT ALTER ANY ROLE TO Admin1;  -- Allow Admin1 (A001) to alter roles
GRANT ALTER ANY ROLE TO Admin2;  -- Allow Admin2 (A002) to alter roles
...
CREATE OR ALTER PROCEDURE sp_AddLecturer
    @LecturerLogin VARCHAR(256),      -- The lecturer's login ID (e.g., L005)
...
BEGIN
    ...
    VALUES (
        ...
        ENCRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), @StudentPwd),   -- Encrypt password
        @StudentName,
        @StudentPhone
    );
END;
...
AS
BEGIN
    -- Apply masking logic directly within the stored procedure
    SELECT 
        ID,
        StudentID,
        LecturerID,
        ...
        ValidTo
    FROM ResultHistory
    WHERE ValidTo <> '9999-12-31 23:59:59.9999999';  -- Identifies deleted records
END;
GO
...
BEGIN
    ...
    SELECT 
		...
		CONVERT(VARCHAR(100), DECRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), SystemPwd)) AS Decrypted_Password,
        Phone AS Unmasked_Phone, 
        Department
    FROM 
        Lecturer
...
CREATE OR ALTER PROCEDURE SP_Lecturer_UpdateDetails
    ...
    @NewPwd VARCHAR(max) = NULL  -- Use VARCHAR for the plain text password
AS
BEGIN
	-- Step 1: Get the current user's login (SYSTEM_USER gives us the SQL Server login)
    DECLARE @LecturerLogin VARCHAR(256);
    DECLARE @CurrentPwd VARCHAR(max);    -- Variable to store the current decrypted password
    ...
    BEGIN
        ...
        SELECT @CurrentPwd = CONVERT(VARCHAR(max), DECRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), SystemPwd))
        FROM Lecturer
        WHERE CONVERT(VARCHAR(10), DECRYPTBYKEY(ID)) = @LecturerLogin;
        ...
        BEGIN
            ...
            UPDATE Lecturer
            SET SystemPwd = ENCRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), CONVERT(VARCHAR(max), @NewPwd))
            WHERE CONVERT(VARCHAR(256), DECRYPTBYKEY(ID)) = @LecturerLogin;
    ...
    DECRYPTION BY PASSWORD = 'LecturerIDEncryptionKey@456';
    SELECT 
        CONVERT(VARCHAR(max), DECRYPTBYKEY(ID)) AS Decrypted_LecturerID, 
		CONVERT(VARCHAR(max), DECRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), SystemPwd)) AS Decrypted_Password,
        Name, 
        Phone AS Unmasked_Phone, 
		Department
    FROM Lecturer
    WHERE CONVERT(VARCHAR(max), DECRYPTBYKEY(ID)) = @LecturerLogin;
...
BEGIN
    ...
    -- Select all results from the Result table, along with 
	-- student, subject, and lecturer information
    SELECT 
        r.ID AS ResultID,
...
ADD BLOCK PREDICATE dbo.fn_LecturerSecurityPredicate(ID) ON dbo.Lecturer AFTER UPDATE; -- Prevent lecturers from updating data for other lecturers
GO
...
SELECT ID,
       CONVERT(VARCHAR(100), DECRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), SystemPwd)) AS DecryptedPassword,
       Name,
       Phone
FROM Student
...
CREATE OR ALTER PROCEDURE sp_UpdateStudentDetails
    ...
    @NewPwd VARCHAR(max) = NULL         -- New password (optional)
AS
BEGIN
    -- Step 1: Get the current user's login (SYSTEM_USER gives us the SQL Server login)
    DECLARE @StudentLogin VARCHAR(100);
    ...
    SELECT 
        ...
        Name, 
        Phone, 
        CONVERT(VARCHAR(max), DECRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), SystemPwd)) AS DecryptedPassword
    FROM Student
    WHERE ID = @StudentLogin;
END;
GO
...
AS
RETURN (
    -- The function returns true if the current student's ID matches the logged-in user's ID
    SELECT 1 AS fn_student_security_predicate_result
    WHERE @StudentID = SYSTEM_USER  -- Ensure that the StudentID matches the SYSTEM_USER (current logged-in user)
);
GO
