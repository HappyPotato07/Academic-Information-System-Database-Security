...
SELECT 
    ID,                      -- Student ID
	CONVERT(VARCHAR(max), DECRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), SystemPwd)) AS DecryptedPassword,  -- Decrypt password
    Name,                    -- Student Name
...
SELECT 
    CONVERT(VARCHAR(MAX), DECRYPTBYKEY(ID)) AS DecryptedLecturerID,   -- Decrypted Lecturer ID
...
---- Step 1: Open the symmetric key for decryption of LecturerID
--OPEN SYMMETRIC KEY LecturerIDKey DECRYPTION BY PASSWORD = 'LecturerIDEncryptionKey@456';
--GO

---- Step 2: Update the password for the lecturer with the name 'Lecturer1'
--UPDATE Lecturer
--SET SystemPwd = ENCRYPTBYASYMKEY(ASYMKEY_ID('MyRSAKey'), CONVERT(VARCHAR(MAX), 'Lecturer123'))  -- New encrypted password
--WHERE Name = 'Lecturer1';
--GO


--Delete from lecturer where name = 'Lecturer3';
